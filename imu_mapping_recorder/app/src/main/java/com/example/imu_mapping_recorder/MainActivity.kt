...
import com.example.imu_mapping_recorder.sync.BluetoothSyncService
import com.example.imu_mapping_recorder.sync.BtConnectionState
import com.example.imu_mapping_recorder.sync.DeviceRole
import com.example.imu_mapping_recorder.sync.SensorTileConnectionState
import com.example.imu_mapping_recorder.sync.SensorTileService
import com.example.imu_mapping_recorder.sync.SyncReport
import com.example.imu_mapping_recorder.ui.theme.ImuMappingRecorderTheme
...
fun RecordingScreen(
    ...
    var syncTapInProgress by remember { mutableStateOf(false) }
    val syncTapLog = remember { mutableStateListOf<Long>() }
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        if (syncTapInProgress) {
            Card(
                ...
                Row(Modifier.padding(12.dp), verticalAlignment = Alignment.CenterVertically) {
                    Text(
                        "⚡  Tap both phones together — press Done when finished",
                        style = MaterialTheme.typography.bodyMedium,
        ...
        Button(
            onClick = {
                if (!syncTapInProgress) {
                    syncTapInProgress = true
                } else {
                    onSyncTap()
                    syncTapLog.add(System.currentTimeMillis() - recordingStartMs)
                    syncTapInProgress = false
                }
            },
            enabled = isRecording,
            ...
            colors = ButtonDefaults.buttonColors(
                containerColor = if (syncTapInProgress) Color(0xFFFFA000)
                                 else MaterialTheme.colorScheme.secondary
            )
        ) {
            Text(
                if (syncTapInProgress) "DONE ✓" else "SYNC TAP ⚡",
                fontSize = 18.sp,
                fontWeight = FontWeight.Bold,
                color = if (syncTapInProgress) Color.Black else Color.Unspecified
            )
        }
        if (syncTapLog.isNotEmpty()) {
            Column(modifier = Modifier.fillMaxWidth(), verticalArrangement = Arrangement.spacedBy(2.dp)) {
                syncTapLog.forEachIndexed { i, elapsedMs ->
                    val min = elapsedMs / 60000
                    val sec = (elapsedMs % 60000) / 1000
                    Text(
                        "  ✓ Tap ${i + 1}  at +%d:%02d".format(min, sec),
                        style = MaterialTheme.typography.bodySmall,
                        color = MaterialTheme.colorScheme.onSurfaceVariant
                    )
                }
        ...
        Column(Modifier.padding(16.dp)) {
            ...
            if (role == DeviceRole.CONTROLLER) {
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    ...
                    if (onChangeDevice != null && state == BtConnectionState.READY) {
...
fun SensorTileCard(
    ...
    Card(modifier = Modifier.fillMaxWidth()) {
        Column(Modifier.padding(16.dp)) {
            Text("SensorTile.box PRO  (optional)", style = MaterialTheme.typography.titleMedium)
            Spacer(Modifier.height(8.dp))
            Row(verticalAlignment = Alignment.CenterVertically) {
            ...
            if (state == SensorTileConnectionState.IDLE) {
                Text(
                    "Skip if using ST BLE Sensor app separately",
                    style = MaterialTheme.typography.bodySmall,
                    color = MaterialTheme.colorScheme.onSurfaceVariant
                )
            }
